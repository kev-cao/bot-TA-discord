...
const getReactionsOfInactives = (client, message) => {
	...
	let indexOfInactives = topics.map(t => {
		return t[1] ? null : ind++;
	});
	let filteredIndexOfInactives = indexOfInactives.filter(maybeNum => maybeNum != null);
	...
	let inactiveReactions = filteredIndexOfInactives.map(ind => arrReactions[ind]);
	return inactiveReactions;
}

const removeReactionToInactiveTopic = (client, message) => {
	let inactiveReactions = getReactionsOfInactives(client, message);
	let inactiveRoles = inactiveReactions.map(r => {
...
const updateTopicMessage = async (client, guild) => {
  ...
  if (topics.length === 0 && topicsMsg) {
    ...
    removeReactionToInactiveTopic(client, topicsMsg);
  }
